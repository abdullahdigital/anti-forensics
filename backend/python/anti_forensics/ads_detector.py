"""
Alternate Data Stream (ADS) Detector - WORKING VERSION
Optimized for your Flask-based anti-forensics project
"""
...
class ADSDetector:
    ...
    def _init_win32api(self):
        """Initialize pywin32 if available"""
        try:
            import win32file
            self.win32file = win32file
            self.win32api_available = True
            logger.info("pywin32 initialized successfully")
        except ImportError:
            logger.warning("pywin32 not installed. Using PowerShell method only.")
            self.win32api_available = False
    ...
    def _analyze_single_file(self, file_path: str, selected_detectors: List[str] = None) -> Dict[str, Any]:
        ...
        if "powershell" in selected_detectors:
            logger.debug("Using PowerShell detection method...")
            ps_streams = self._detect_with_powershell(file_path)
            if ps_streams:
                for stream in ps_streams:
                    if not self._stream_exists(stream, detected_streams):
                        detected_streams.append(stream)
                detection_methods.append("powershell")
                results["analysis_methods"].append({
                    "method": "powershell",
                    "streams_found": len(ps_streams)
                })
        
        # Method 2: Win32 API
        if "win32api" in selected_detectors and self.win32api_available:
            logger.debug("Using Win32 API detection method...")
            win32_streams = self._detect_with_win32api(file_path)
            if win32_streams:
                for stream in win32_streams:
                    if not self._stream_exists(stream, detected_streams):
                        detected_streams.append(stream)
                detection_methods.append("win32api")
                results["analysis_methods"].append({
                    "method": "win32api",
                    "streams_found": len(win32_streams)
                })
        
        # Method 3: Pattern matching (direct file access)
    ...
    def _detect_with_powershell(self, file_path: str) -> List[ADSStream]:
        """
        Detect ADS using PowerShell - MOST RELIABLE METHOD
        """
        streams = []
        ...
        try:
            # PowerShell command to get all streams
            ps_command = f'''
            $file = "{file_path}"
            if (Test-Path $file) {{
                try {{
                    $streams = Get-Item $file -Stream * -ErrorAction Stop
                    foreach ($stream in $streams) {{
                        $streamName = $stream.Stream
                        if ($streamName -ne "::$DATA") {{
                            $streamPath = "$file`:$streamName"
                            $size = 0
                            try {{
                                $size = (Get-Item $streamPath -ErrorAction SilentlyContinue).Length
                            }} catch {{ }}
                            [PSCustomObject]@{{
                                Stream = $streamName
                                Size = $size
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for line in lines:
                    ...
                    # Expected format: StreamName Size
                    parts = line.split()
                    if len(parts) >= 2:
                        stream_name = parts[0]
                        try:
                        ...
                        stream = ADSStream(
                            name=stream_name,
                            full_path=f"{file_path}:{stream_name}",
                            size_bytes=size,
                            stream_type=self._classify_stream(stream_name)
                        )
                        streams.append(stream)
            
        except subprocess.TimeoutExpired:
            logger.warning(f"PowerShell command timed out for {file_path}")
        except Exception as e:
            logger.debug(f"PowerShell detection failed: {str(e)}")
        
        return streams
    
    def _detect_with_win32api(self, file_path: str) -> List[ADSStream]:
        """Detect ADS using Windows Win32 API"""
        streams = []
        
        if not self.win32api_available:
            return streams
        ...
        try:
            ...
            find_handle = win32file.FindFirstStreamW(file_path)
            while True:
                try:
                    stream_data = win32file.FindNextStreamW(find_handle)
                    if not stream_data:
                        break
                    
                    stream_name = stream_data[1]
                    
                    # Skip default $DATA stream
                    if stream_name == "::$DATA":
                        continue
                    
                    # Construct full stream path
                    stream_full_path = f"{file_path}{stream_name}"
                    
                    # Try to get stream size
                    size = 0
                    try:
                    ...
                    if "No more files" in str(e) or "No data is available" in str(e):
                        break
                    continue
            
            win32file.FindClose(find_handle)
            
        except Exception as e:
            logger.debug(f"Win32 API detection failed: {str(e)}")
    ...
    def _analyze_stream(self, stream: ADSStream) -> ADSStream:
        ...
        try:
            ...
            if 0 < stream.size_bytes <= (MAX_ADS_SIZE_MB * 1024 * 1024):
                try:
    ...
    def _non_windows_response(self, file_path: str) -> Dict[str, Any]:
        ...
        return {
            "file_path": file_path,
            "ads_found": False,
            "note": "Alternate Data Streams are Windows NTFS-specific",
            "platform": platform.system(),
            "timestamp": datetime.now().isoformat(),
            "success": True
        }
...
def create_test_ads_file(file_path: str = "test_ads_demo.txt") -> Tuple[bool, str]:
    ...
    if platform.system() != "Windows":
        return False, "Test only works on Windows"
